...
class FtpProtocol implements ServerProtocol, Traceable {
  ...
  public function openDatasock($socket) {
    ...
    if ($this->datasock[$socket->hashCode()] instanceof \peer\ServerSocket) {
  ...
  public function onPass($socket, $params) {
    with ($user= $this->sessions[$socket->hashCode()]->getUsername()); {
      ...
      } catch (\security\auth\AuthenticatorException $e) {
  ...
  public function onMkd($socket, $params) {
    ...
    // Invoke interceptor
    $entry= $this->storage->createEntry($socket->hashCode(), $params, Storage::COLLECTION);
    if (!$this->checkInterceptors($socket, $entry, 'onCreate')) return;

    // Create the element
    try {
      $this->storage->create($socket->hashCode(), $params, Storage::COLLECTION);
    } catch (\lang\XPException $e) {
  ...
  public function onStor($socket, $params) {
    ...
    if (!($entry= $this->storage->lookup($socket->hashCode(), $params))) {
      ...
      // Invoke interceptor
      $entry= $this->storage->createEntry($socket->hashCode(), $params, Storage::ELEMENT);
      ...
      try {
        $entry= $this->storage->create($socket->hashCode(), $params, Storage::ELEMENT);
      } catch (\lang\XPException $e) {
        $this->answer($socket, 550, $params.': '.$e->getMessage());
        $dataSocket->close();
        return;
